...
* 能够独立部署`linux`开发环境（lamp / lnmp），熟悉常用的`shell`操作，日常使用`nvim`作为开发工具，熟练操作各种命令行场景
* 有`redis`，`kafka`使用经验（解决缓存，跨业务模块通信）
* 有`elk`套件的使用经验（解决公司项目的日志检索问题）
* 有`laravel`开发经验

## AI 开发提效经验
* 基于腾讯 codebuddy 智能体编写配置，根据需求完成开发，提测，部署等一系列自动化工作
	* 结合司内平台，已有的sdk/api 等能力包装为mcp，skill，使agent可以对接公司内各个平台
  	* 开发机上的agent常驻进程，对接企微机器人，实现远程完成coding和部署等操作
  	* 利用agent开发多agent web控制面板，基于acp协议对接，实现在一个界面内控制多个agent对话，并有优于企业微信的交互体验
  	* 基于figma-mcp + 自定义规则快速完成web前端/小程序界面开发
* 通过agent对接公司内日志平台 + 自定义规则，快速定位生产环境中的报错源头，并不断完善优化规则，提高速度和精准度
